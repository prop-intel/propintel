...
const SCORE_WEIGHTS = {
  aeo: 0.5, // 50% weight for AEO visibility
  llmeo: 0.3, // 30% weight for LLMEO
  seo: 0.2, // 20% weight for SEO
};
...
export function generateMarkdownReport(report: AEOReport): string {
  ...
  md.push(`|--------|-------|-------|`);
  md.push(
    `| **AEO Visibility** | ${report.scores.aeoVisibilityScore}/100 | ${getGrade(report.scores.aeoVisibilityScore)} |`,
  );
  md.push(
    `| LLMEO Score | ${report.scores.llmeoScore}/100 | ${getGrade(report.scores.llmeoScore)} |`,
  );
  md.push(
    `| SEO Score | ${report.scores.seoScore}/100 | ${getGrade(report.scores.seoScore)} |`,
  );
  md.push(
    `| **Overall** | ${report.scores.overallScore}/100 | ${getGrade(report.scores.overallScore)} |`,
  );
  md.push("");
  ...
  if (report.aeoAnalysis.competitors.length > 0) {
    ...
    report.aeoAnalysis.competitors.slice(0, 5).forEach((c) => {
      md.push(
        `| ${c.domain} | ${Math.round(c.citationRate)}% | ${c.averageRank.toFixed(1)} |`,
      );
    });
    md.push("");
  }
  ...
  const highPriority = report.aeoRecommendations.filter(
    (r) => r.priority === "high",
  );
  const mediumPriority = report.aeoRecommendations.filter(
    (r) => r.priority === "medium",
  );
  ...
  if (highPriority.length > 0) {
    ...
    highPriority.forEach((r, i) => {
      ...
      if (r.competitorExample) {
        md.push(
          `> Reference: ${r.competitorExample.domain} - ${r.competitorExample.whatTheyDoBetter}`,
        );
      }
      md.push("");
    });
  ...
  md.push("");
  md.push(
    "Copy the following prompt into Cursor to implement the recommended changes:",
  );
  md.push("");
  md.push("```");
  ...
  md.push(
    `*Pages analyzed: ${report.meta.pagesAnalyzed} | Confidence: ${Math.round(report.scores.confidence * 100)}%*`,
  );
...
function calculateConfidence(
  ...
  const citationFactor =
    citationRate > 0 ? Math.min(citationRate / 50, 1) : 0.5;

  return (
    Math.round(
      (pagesFactor * 0.3 + queriesFactor * 0.4 + citationFactor * 0.3) * 100,
    ) / 100
  );
}
