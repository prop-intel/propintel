...
const WEIGHTS_LEGACY = {
  ...
  competitivePosition: 0.2, // How you compare to competitors
  queryBreadth: 0.1, // Coverage across different query types
  gapPenalty: 0.1, // Penalty for missed opportunities
};

// Score weights WITH GEO (new mode)
const WEIGHTS_WITH_GEO = {
  citationRate: 0.28, // How often you appear in search (reduced)
  rankQuality: 0.2, // Position when you do appear (reduced)
  ...
  geoScore: 0.2, // NEW: LLM brand recognition score
};
...
export async function calculateVisibilityScore(
  ...
  try {
    ...
    const competitiveScore = safeNumber(
      calculateCompetitiveScore(citationAnalysis, competitors),
    );
    const breadthScore = safeNumber(calculateBreadthScore(citationAnalysis));
    const gapPenalty = safeNumber(
      calculateGapPenalty(citationAnalysis.gaps, citationAnalysis.totalQueries),
    );
...
export function buildAEOAnalysis(
  ...
  return {
    ...
    citationCount:
      citationAnalysis.citedQueries + citationAnalysis.mentionedQueries,
...
function calculateBreadthScore(analysis: CitationAnalysisResult): number {
  ...
  if (analysis.queryTypesWinning instanceof Map) {
    ...
    analysis.queryTypesWinning &&
    typeof analysis.queryTypesWinning === "object"
  ) {
...
function generateScoreSummary(
  ...
  if (score >= 70) {
    ...
    parts.push(
      `Moderate AEO visibility with a score of ${score}/100. Significant room for improvement.`,
    );
  } else {
    parts.push(
      `Low AEO visibility with a score of ${score}/100. Immediate attention needed.`,
    );
  }
  ...
  if (topContributor) {
    ...
    parts.push(
      `Your strongest area is ${formatComponentName(key)} (contributing ${value.contribution} points).`,
    );
  }
  ...
  if (breakdown.gapPenalty.contribution < -10) {
    parts.push(
      `Missing opportunities are hurting your score (${Math.abs(breakdown.gapPenalty.contribution)} point penalty).`,
    );
  }
  ...
  if (geoScore !== undefined) {
    if (geoScore >= 70) {
      parts.push(
        `Strong LLM brand recognition (GEO: ${geoScore}/100) - AI models know your brand.`,
      );
    } else if (geoScore >= 40) {
      parts.push(
        `Moderate LLM brand recognition (GEO: ${geoScore}/100) - room to improve AI visibility.`,
      );
    } else if (geoScore > 0) {
      parts.push(
        `Low LLM brand recognition (GEO: ${geoScore}/100) - AI models rarely mention your brand.`,
      );
    } else {
      parts.push(
        `Critical: AI models don't recognize your brand (GEO: 0/100).`,
      );
    }
  }
