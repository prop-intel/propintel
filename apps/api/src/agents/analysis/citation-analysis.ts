...
export async function analyzeCitationPatterns(
  ...
  const span = trace.span({
    ...
    input: { citationCount: citations.length },
  });
...
function identifyGaps(
  ...
  for (const citation of citations) {
    ...
    if (citation.yourPosition === 'absent' || (citation.yourRank && citation.yourRank > 5)) {
      const searchResult = searchResults.find(r => r.query === citation.query);
...
function generateFindings(
  ...
  if (metrics.citationRate >= 70) {
    findings.push(`Strong visibility: Your domain appears in ${Math.round(metrics.citationRate)}% of relevant searches`);
  } else if (metrics.citationRate >= 40) {
    findings.push(`Moderate visibility: Your domain appears in ${Math.round(metrics.citationRate)}% of relevant searches - room for improvement`);
  } else {
    findings.push(`Low visibility: Your domain only appears in ${Math.round(metrics.citationRate)}% of relevant searches`);
  }

  // Top 3 ranking finding
  if (metrics.top3Rate >= 50) {
    findings.push(`Strong rankings: You rank in top 3 for ${Math.round(metrics.top3Rate)}% of queries`);
  } else if (metrics.top3Rate > 0) {
    findings.push(`Ranking opportunity: Only ${Math.round(metrics.top3Rate)}% of queries have you in top 3`);
  }

  // Query type patterns
  const bestPerformingType = [...patterns.winning.entries()]
    .sort((a, b) => b[1] - a[1])[0];
  const worstPerformingType = [...patterns.losing.entries()]
    .sort((a, b) => b[1] - a[1])[0];

  if (bestPerformingType) {
    findings.push(`Best performing query type: "${bestPerformingType[0]}" queries (${bestPerformingType[1]} wins)`);
  }
  if (worstPerformingType && worstPerformingType[1] >= 2) {
    findings.push(`Weakest query type: "${worstPerformingType[0]}" queries (missing ${worstPerformingType[1]} opportunities)`);
  }
