...
import { type PageAnalysis, type TargetQuery, type CompetitorVisibility } from '../../types';
...
const GeneratedPromptsSchema = z.object({
  prompts: z.array(z.object({
    ...
    category: z.enum(['direct', 'category', 'problem', 'use_case', 'comparison', 'industry', 'recommendation'])
...
const MentionAnalysisSchema = z.object({
  ...
  competitorsMentioned: z.array(z.object({
    name: z.string(),
    mentioned: z.boolean(),
    mentionType: z.enum(['primary', 'listed', 'compared', 'indirect']).optional(),
...
export async function probeLLMsForBrand(
  ...
  const { promptCount = DEFAULT_PROMPT_COUNT, models = [...DEFAULT_MODELS] } = options;
  ...
  try {
    ...
    const prompts = await generateProbePrompts(input, promptCount, tenantId, jobId);
...
${input.targetQueries.slice(0, 8).map(q => `- ${q.query} (${q.type})`).join('\n')}
...
async function probeModel(
  ...
  for (let i = 0; i < prompts.length; i += PROBE_CONCURRENCY) {
    ...
    const batchResults = await Promise.all(
      batch.map(prompt => probeSinglePrompt(model, prompt, brand, competitors, tenantId, jobId))
    );
...
async function probeSinglePrompt(
  ...
  try {
    ...
    return {
      ...
      competitorsMentioned: competitors.map(c => ({ name: c, mentioned: false })),
      probedAt: new Date().toISOString(),
    };
  }
}
...
): Promise<Omit<LLMProbeResult, 'model' | 'prompt' | 'promptCategory' | 'response' | 'probedAt' | 'responseTokens'>> {
  // First, do a quick regex check for obvious mentions
  const brandRegex = new RegExp(escapeRegex(brand.name), 'gi');
  const domainRegex = new RegExp(escapeRegex(brand.domain.replace(/^www\./, '')), 'gi');
  
  const hasBrandMention = brandRegex.test(response) || domainRegex.test(response);
...
function aggregateProbeResults(
  ...
  const overallMentionRate = totalProbes > 0 
    ? (mentionedProbes.length / totalProbes) * 100 
    : 0;
  
  const primaryMentionRate = totalProbes > 0
    ? (primaryProbes.length / totalProbes) * 100
    : 0;
  
  // Average position when mentioned
  const positionedResults = results.filter(r => r.mentionPosition !== undefined);
  const averageMentionPosition = positionedResults.length > 0
    ? positionedResults.reduce((sum, r) => sum + (r.mentionPosition || 0), 0) / positionedResults.length
    : 0;
  ...
  for (const model of models) {
    ...
    const modelPrimary = modelResults.filter(r => r.mentionType === 'primary');
    
    modelBreakdown[model] = {
      mentionRate: modelResults.length > 0 ? (modelMentioned.length / modelResults.length) * 100 : 0,
      primaryRate: modelResults.length > 0 ? (modelPrimary.length / modelResults.length) * 100 : 0,
      probeCount: modelResults.length,
    };
  }
  
  // Category breakdown
  const categories: PromptCategory[] = ['direct', 'category', 'problem', 'use_case', 'comparison', 'industry', 'recommendation'];
  const categoryBreakdown: LLMBrandProbeAggregate['categoryBreakdown'] = {} as LLMBrandProbeAggregate['categoryBreakdown'];
  ...
  for (const category of categories) {
    ...
    categoryBreakdown[category] = {
      mentionRate: catResults.length > 0 ? (catMentioned.length / catResults.length) * 100 : 0,
      primaryRate: catResults.length > 0 ? (catPrimary.length / catResults.length) * 100 : 0,
      probeCount: catResults.length,
    };
  }
  
  // Competitor comparison
  const competitorComparison: LLMBrandProbeAggregate['competitorComparison'] = competitors.map(competitor => {
    let theirMentions = 0;
    let yourMentions = 0;
    ...
    for (const result of results) {
      const competitorMention = result.competitorsMentioned.find(c => 
        c.name.toLowerCase() === competitor.toLowerCase()
      );
    ...
    return {
      competitor,
      theirMentionRate: totalProbes > 0 ? (theirMentions / totalProbes) * 100 : 0,
  ...
  const sentimentBreakdown = {
    positive: results.filter(r => r.sentiment === 'positive').length / Math.max(totalProbes, 1),
    neutral: results.filter(r => r.sentiment === 'neutral').length / Math.max(totalProbes, 1),
    negative: results.filter(r => r.sentiment === 'negative').length / Math.max(totalProbes, 1),
  };
  ...
  const sortedCategories = Object.entries(categoryBreakdown)
    .sort((a, b) => b[1].mentionRate - a[1].mentionRate);
...
function calculateGEOScore(
  ...
  // Weight components
  const weights = {
    mentionRate: 0.30,        // How often you appear
    primaryRate: 0.25,        // How often you're the top answer
    position: 0.15,           // Position quality when listed
    categoryBreadth: 0.10,    // Coverage across categories
    competitiveWins: 0.10,    // How you compare to competitors
    sentiment: 0.10,          // Positive vs negative mentions
  };
  ...
  const positionScore = avgPosition > 0 
    ...
    : mentionScore > 0 ? 50 : 0; // Default to 50 if mentioned but no position data
  
  // Category breadth score
  const categoriesWithMentions = Object.values(categoryBreakdown)
    .filter(c => c.mentionRate > 0).length;
  const categoryScore = (categoriesWithMentions / 7) * 100;
  
  // Competitive wins score
  const totalWins = competitorComparison.reduce((sum, c) => sum + c.headToHeadWins, 0);
  const totalLosses = competitorComparison.reduce((sum, c) => sum + c.headToHeadLosses, 0);
  const competitiveScore = totalWins + totalLosses > 0
    ? (totalWins / (totalWins + totalLosses)) * 100
    : 50; // Neutral if no competitors
  
  // Sentiment score
  const sentimentScore = (sentimentBreakdown.positive * 100) + 
...
function generateKeyFindings(
  ...
  if (overallRate >= 70) {
    findings.push(`Strong LLM visibility: Brand appears in ${Math.round(overallRate)}% of relevant prompts`);
  } else if (overallRate >= 40) {
    findings.push(`Moderate LLM visibility: Brand appears in ${Math.round(overallRate)}% of relevant prompts`);
  } else if (overallRate > 0) {
    findings.push(`Low LLM visibility: Brand only appears in ${Math.round(overallRate)}% of relevant prompts`);
  } else {
    findings.push('Critical: Brand not recognized by LLMs in any tested prompts');
  }
  
  // Primary recommendation rate
  if (primaryRate >= 30) {
    findings.push(`Excellent brand authority: Primary recommendation in ${Math.round(primaryRate)}% of responses`);
  } else if (primaryRate < 10 && overallRate > 30) {
    findings.push(`Listed but rarely primary: Consider content that positions brand as the top solution`);
  }
  
  // Category insights
  const bestCategory = Object.entries(categoryBreakdown)
    .sort((a, b) => b[1].mentionRate - a[1].mentionRate)[0];
  ...
  if (bestCategory && bestCategory[1].mentionRate > 50) {
    findings.push(`Strongest in ${formatCategoryName(bestCategory[0] as PromptCategory)} (${Math.round(bestCategory[1].mentionRate)}% mention rate)`);
  }
  
  if (worstCategory && worstCategory[1].mentionRate < 20 && worstCategory[1].probeCount >= 3) {
    findings.push(`Gap in ${formatCategoryName(worstCategory[0] as PromptCategory)} - only ${Math.round(worstCategory[1].mentionRate)}% mention rate`);
  }
  
  // Competitor insights
  const beatingYou = competitorComparison.filter(c => c.theirMentionRate > overallRate + 10);
  if (beatingYou.length > 0) {
    const topCompetitor = beatingYou.sort((a, b) => b.theirMentionRate - a.theirMentionRate)[0];
    if (topCompetitor) {
      findings.push(`${topCompetitor.competitor} has higher LLM visibility (${Math.round(topCompetitor.theirMentionRate)}% vs your ${Math.round(overallRate)}%)`);
    }
  }
  
  // Sentiment
  if (sentiment.negative > 0.1) {
    findings.push(`Warning: ${Math.round(sentiment.negative * 100)}% of mentions have negative sentiment`);
  } else if (sentiment.positive > 0.5) {
    findings.push(`Positive brand perception: ${Math.round(sentiment.positive * 100)}% of mentions are positive`);
  }
