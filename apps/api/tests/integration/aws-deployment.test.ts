...
describeIfAws("AWS Deployment Verification", () => {
  ...
  describe("Deployed API Health", () => {
    it("should respond to health check", async () => {
      ...
      const response = await makeBackendApiRequest(AWS_API_URL, "/health", {
        method: "GET",
      });
  ...
  describe("Authentication on AWS", () => {
    it("should authenticate with API key on deployed backend", async () => {
      ...
      const response = await makeBackendApiRequest(AWS_API_URL, "/jobs", {
        method: "GET",
        apiKey: AWS_API_KEY,
      });
  ...
  describe("Schema Import Paths on AWS", () => {
    it("should handle schema imports in Lambda environment", async () => {
      if (!AWS_API_URL) return;

      // This test verifies that the schema imports work in the Lambda environment
      // If they don't, job creation would fail with import errors
  ...
  describe("Environment Variables", () => {
    it("should have required environment variables set", () => {
      ...
      const requiredVars = ["DATABASE_URL", "OPENAI_API_KEY", "TAVILY_API_KEY"];
